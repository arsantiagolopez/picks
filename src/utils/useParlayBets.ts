...
interface Response {
  ...
  setBets: Dispatch<SetStateAction<ParlayBetEntity[]>>;
}
...
const useParlayBets = ({
  ...
  const [bets, setBets] = useState<ParlayBetEntity[]>([]);
  const [query, setQuery] = useState<string | null>(null);

  const { data } = useSWR<ParlayBetEntity[]>(
    query && `/api/parlayBets?set=${query}`
  );
